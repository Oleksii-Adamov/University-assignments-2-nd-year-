...
void tsort(StrAdj, int []);
void tops(StrAdj, int, int [], int &);

bool num[MaxN];

int main(){
  StrAdj s;
  int cnm[MaxN], label[MaxN];
  bld_gr(s);
  ...
  tsort(s, label);
  cout << "Numeration vertex in topological sorting: " << endl;
  for (int v=0; v<s.n; v++) cout << label[v] << " ";
  cout << endl;
  system("pause");
  return 0;
}
...
//топологічне сортування на графі

void tsort(StrAdj s, int label[MaxN]){
	int v, c;
	for (v=0; v<MaxN; v++){
    num[v]=v<s.n; label[v]=0;}
