...
#include <random>
using namespace std;
const int SIZE = 100, TESTS = 100, NUMBER_OF_SORTS = 5;
long long copies = 0;
struct sort_algo {
...
long double ex_time_of_sort(int a[], int mode) {
	...
	if (mode == 3) merge_sort(t, SIZE);
	if (mode == 4) heapSort(t, SIZE);
...
int main() {
	...
	for (int mode = 0; mode < NUMBER_OF_SORTS; mode++) {
		...
		if (mode == 3) sort_algos[mode].name = "merge sort";
		if (mode == 4) sort_algos[mode].name = "heap sort";
	}
	int t = TESTS;
	while (t--) {
		std::random_device rd; 
		...
		//cout << '\n';
		for (int mode = 0; mode < NUMBER_OF_SORTS; mode++) {
			sort_algos[mode].nano_sec += ex_time_of_sort(a, mode) / SIZE;
			sort_algos[mode].num_of_copies += long double(copies) / SIZE;
		}
		delete[] a;
	}
