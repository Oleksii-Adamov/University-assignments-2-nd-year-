...
#include <ctime>
#include <iostream>
const double multipler_of_a = 1e3;
bool hash_table::is_prime(long long num) {
...
long long hash_table::next_prime_after_num_by_multiplier(size_t num) {
	long long n = num;
	//n *= multipler_of_a;
	long long initial_step = (n % 2) + 1;
	for (long long i = n + initial_step; ; i += 2) {
...
size_t hash_table::universal_hash_function(size_t a, size_t b, size_t m, double value) {
	...
	char c;
	//std::cin >> c;
	return (long long(abs(a * multipler_of_a * value + b)) % p) % m;
}
void hash_table::init(size_t hash_table_size, double* arr, size_t array_size) {
	// initialization
	m_size = hash_table_size;
	p = next_prime_after_num_by_multiplier(array_size);
	std::cout << p << "\n";
		...
		if (m_table[main_index][additional_index] == nullptr) {
			...
			while (collisions) {
				...
				if (m_table[main_index][index] == nullptr) {
					...
					std::cout << arr[i] << " " << *(m_table[main_index][index]) << " " << index  << " " 
