...
universal_hash_vector_double::universal_hash_vector_double(unsigned long long i_a, unsigned long long i_b, unsigned long long i_k) {
	set(i_a, i_b, i_k);
}
void universal_hash_vector_double::set(unsigned long long i_a, unsigned long long i_b, unsigned long long i_k) {
	a = i_a;
	b = i_b;
	k = i_k;
}
void universal_hash_vector_double::random(std::mt19937& gen, std::uniform_int_distribution<>& dis_a,
	std::uniform_int_distribution<>& dis_b, std::uniform_int_distribution<>& dis_k) {
	a = dis_a(gen);
	b = dis_b(gen);
	k = dis_k(gen);
}
